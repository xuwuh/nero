...
void model_tests(){
    ...
    config.conv_out_channels = 2;
    config.conv_kernel_h = 2;
    config.conv_kernel_w = 2;
    config.conv_stride = 1;
    config.conv_padding = 0;

    config.pool_size = 2;
    config.pool_stride = 2;

    config.learning_rate = 0.01;
    config.regularizator = 0.0001;
    config.momentum = 0.9;

    if (!model_init(&model, &config)) {printf("ERROR: model init test failed\n");return;}
    ...
    tensor_set(&input, 0, 0, 1, 0, 5.0);
    tensor_set(&input, 0, 0, 1, 1, 6.0);
    tensor_set(&input, 0, 0, 1, 2, 7.0);
    tensor_set(&input, 0, 0, 1, 3, 8.0);
    tensor_set(&input, 0, 0, 2, 0, 9.0);
    tensor_set(&input, 0, 0, 2, 1, 10.0);
    tensor_set(&input, 0, 0, 2, 2, 11.0);
    tensor_set(&input, 0, 0, 2, 3, 12.0);
    tensor_set(&input, 0, 0, 3, 0, 13.0);
    tensor_set(&input, 0, 0, 3, 1, 14.0);
    tensor_set(&input, 0, 0, 3, 2, 15.0);
    tensor_set(&input, 0, 0, 3, 3, 16.0);

    conv_out=tensor_create(1, config.conv_out_channels, model.conv_out_h, model.conv_out_w);
    relu_out=tensor_create(1, config.conv_out_channels, model.conv_out_h, model.conv_out_w);
    pool_out=tensor_create(1, config.conv_out_channels, model.pool_out_h, model.pool_out_w);
    flat_out=matrix_create(1, model.flatten_size);
    logits=matrix_create(1, config.classes);
    probabilities=matrix_create(1, config.classes);
    dlogits=matrix_create(1, config.classes);
    labels[0]=1;
    if (!model_forward(&model, &input, &conv_out, &relu_out, &pool_out, &flat_out, &logits, &probabilities)){printf("ERROR: model forward failed\n");
        tensor_free(&input);
    ...
    printf("test model loss: %.6f\n", loss);
    matrix_print(&probabilities, "test model probabilities");

    if (!backward(&probabilities, labels, &dlogits)){printf("ERROR: model loss backward failed\n");}
    if (!model_backward(&model, &input, &conv_out, &relu_out, &pool_out, &flat_out, &dlogits)){printf("ERROR: model backward failed\n");}
    if (!model_sgd(&model, config.learning_rate, config.regularizator)){printf("ERROR: model update failed\n");}

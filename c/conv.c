...
//инициализация слоя
int conv_init(Conv *layer, int in_c, int out_c, int kernel_h, int kernel_w, int sride, int padding){
    layer->in_c=in_c;
    layer->out_c=out_c;
    ...
    layer->velociry_weights=matrix_create(weight_rows, weight_cols);
    layer->velociry_bias=matrix_create(1, out_c);

    conv_init_weights(&layer->weights);
    matrix_zero(&layer->bias);
    matrix_zero(&layer->dweights);
    matrix_zero(&layer->dbias);
    matrix_zero(&layer->velociry_weights);
    matrix_zero(&layer->velociry_bias);
    return 1;
}
...
int conv_forward(Conv *layer, Tensor *input, Tensor *output){
    ...
    if (!im2col(input, &input_col, layer->kernel_h, layer->kernel_w, layer->sride, layer->padding)){
...
int conv_backward(Conv *layer, Tensor *input, Tensor *doutput, Tensor *dinput){
    ...
    im2col(input, &input_col, layer->kernel_h, layer->kernel_w, layer->sride, layer->padding);
    ...
    col2im(&dinput_col, dinput, layer->kernel_h, layer->kernel_w, layer->sride, layer->padding);
...
//градиент
void conv_sgd(Conv *layer, double learning_rate, double regularization){
...
//градиент с моментум
void conv_momentum(Conv *layer, double learning_rate, double momentum, double regularization){
    optimizer_sgd_momentum_l2(&layer->weights, &layer->dweights, &layer->velociry_weights, learning_rate, momentum, regularization);
    optimizer_sgd_momentum_l2(&layer->bias, &layer->dbias, &layer->velociry_bias, learning_rate, momentum, 0.0);
}
...
void conv_free(Conv *layer){
    ...
    matrix_free(&layer->velociry_weights);
    matrix_free(&layer->velociry_bias);
